...
interface ClientInfoFormProps {
  onClientSelect?: (clientId: string | null, clientName?: string, phcNumber?: string, contactName?: string) => void;
}
...
export const ClientInfoForm: React.FC<ClientInfoFormProps> = ({
  ...
  const handleClientSelection = async (client: {
    ...
    if (onClientSelect) {
      onClientSelect(client.id, client.client_name, client.phc_number, client.contact_name || '');
    }
  ...
  const handleContactSelect = (contact: Contact) => {
    ...
    if (onClientSelect) {
      onClientSelect(contact.id, contact.client_name, contact.phc_number, contact.contact_name || '');
    }
  };
  ...
  const addNewContact = async () => {
    ...
    try {
      ...
      setClientId(data.id);
      if (onClientSelect) onClientSelect(data.id, data.client_name, data.phc_number, data.contact_name || '');
    ...
    try {
      ...
      if (existingClients && existingClients.length > 0) {
        ...
        if (clientId) {
          ...
          if (onClientSelect) onClientSelect(existingClients[0].id, existingClients[0].client_name, existingClients[0].phc_number, existingClients[0].contact_name || '');
        }
      } else {
